...
//                Runnable pageSaving = () -> {
                ...
                if (!pageEntityList.contains(pageEntity)) {
                        ...
                        double revelevance = (double) pageRelevance(pageEntity);

                        PageData pageData = new PageData(site,siteName,url,tittle,snippet,revelevance );
...
//            }
    ...
    public boolean  isGoodLemma(int frequency,int allLemmaInSite){
        if(frequency/allLemmaInSite <0.75){
           return true;
        }
        return false;
